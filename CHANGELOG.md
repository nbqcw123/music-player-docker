...
### 改动
- 🎨 **全局搜索**：移除音源选择标签，搜索不显示来源
- 💿 **歌词界面**：正在播放页面显示歌词（中间大封面+下方歌词）
- ⬇️ **下载按钮**：播放栏增加下载按钮，点击直接下载当前歌曲
- 📌 **版本号**：Logo 旁显示版本号（动态从 API 获取）
- ⚠️ **提示信息**：底部注明"因各大平台限制，部分歌曲可能无法播放"
- 🔧 **新增 /api/lyric 路由**：获取歌词（tonzhon API）
- 📐 **布局优化**：整体更紧凑顺眼，响应式适配
