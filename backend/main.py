...
async def check_url(url: str = Query(...)):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.head(url, headers=headers, timeout=aiohttp.ClientTimeout(total=3), allow_redirects=True) as resp:
                ok = resp.status < 400
...
async function playIdx(i){const s=S.list[i];if(!s)return;const id=s.id;if(S.failed.has(id)){toast("\u8be5\u6b4c\u66f2\u6682\u4e0d\u53ef\u64ad\u653e");if(i<S.list.length-1)playIdx(i+1);return}if(!S.checked.has(id)){toast("\u68c0\u6d4b\u64ad\u653e\u94fe\u63a5\u2026");let u;try{const d=await api("/api/stream",{source:s._source||"netease",id});u=d.stream_url}catch(e){S.failed.add(id);renderSearch();toast("\u94fe\u63a5\u83b7\u53d6\u5931\u8d25");if(i<S.list.length-1)playIdx(i+1);return}if(!u){S.failed.add(id);renderSearch();toast("\u8be5\u6b4c\u66f2\u6682\u4e0d\u53ef\u64ad\u653e");if(i<S.list.length-1)playIdx(i+1);return}if(!await checkUrl(u)){S.failed.add(id);renderSearch();toast("\u8be5\u6b4c\u66f2\u94fe\u63a5\u4e0d\u53ef\u7528");if(i<S.list.length-1)playIdx(i+1);return}S.checked.add(id);s._playUrl=u}else{s._playUrl=s._playUrl||u}_playSong(s,i)}
